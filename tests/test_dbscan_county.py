...
def load_county_aqi_data_pl() -> pl.DataFrame:
    ...
    df = df.to_pandas()
    # NOTE: added 'county Name', 'State Name' to see if getting multiple counties
    df = df[['County Code', 'Date', 'AQI', 'county Name', 'State Name']]
...
def main(county_num: int = 1):
    ...
    # ISSUE: above actually selects multiple counties
    print(county['county Name'].unique())
    print("oops")
